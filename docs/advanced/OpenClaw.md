...
Packy专门为OpenClaw配置写了一个配置脚本，Github地址为：[openclaw-configurator](https://forward.apiapi.site/https://github.com/packyme/openclaw-configurator)，通过此脚本，可以快速帮助我们配置Packy的API。
...
目前脚本还不支持配置Gemini渠道，正在施工中~~~
  ...
  1. 在你的SSH控制台，或者MacOS终端输入以下命令安装配置器
    ...
    curl -fsSL https://forward.apiapi.site/https://github.com/packyme/openclaw-configurator/releases/latest/download/index.js -o /tmp/openclaw-config.js && node /tmp/openclaw-config.js
